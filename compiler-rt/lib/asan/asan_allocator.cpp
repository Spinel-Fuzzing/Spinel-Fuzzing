...
struct Allocator {
  ...
  void *Allocate(uptr size, uptr alignment, BufferedStackTrace *stack,
    ...
    if (size != size_rounded_down_to_granularity && CanPoisonMemory()) {
      ...
      // *shadow = fl.poison_partial ? (size & (ASAN_SHADOW_GRANULARITY - 1)) : 0; // @acesrc: change the asan poisoning
      *shadow = fl.poison_partial ? ((ASAN_SHADOW_GRANULARITY - (size & (ASAN_SHADOW_GRANULARITY - 1)))) : 0;
    }
