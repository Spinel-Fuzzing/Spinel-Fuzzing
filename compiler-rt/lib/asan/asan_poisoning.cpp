...
void __asan_poison_memory_region(void const volatile *addr, uptr size) {
  ...
  // Printf("User Poisoning %p: , size: %llu\n", addr, size);
  ...
  Printf("[+] Poison memory region [%p, %p)\n    ", (void *)beg_addr, (void *)end_addr);
  Printf("%02x | ", *(end.chunk - 1));
  for (u8 *i = end.chunk; i <= end.chunk + 8; i++) Printf("%02x ", *i);
  Printf("\n");
  ...
  if (beg.chunk == end.chunk) {
    ...
    if (value > 0 && value <= 8 && (s8)(ASAN_SHADOW_GRANULARITY - value) >= end.offset) {
      if (beg.offset > 0) {
        *beg.chunk = ASAN_SHADOW_GRANULARITY - Min((s8)(ASAN_SHADOW_GRANULARITY - value), beg.offset); 
      } else {
        *beg.chunk = kAsanUserPoisonedMemoryMagic;
      }
    }
  ...
  if (beg.offset > 0) {
    ...
    if (beg.value == 0) {
      *beg.chunk = ASAN_SHADOW_GRANULARITY - beg.offset;
    } else {
      if (beg.value >= 0 && beg.value <= 8) *beg.chunk = Max(beg.value, (s8)(ASAN_SHADOW_GRANULARITY - beg.offset));
    }
  ...
  // }
  if ((end.value >= 0 && end.value <= 8) && (s8)(ASAN_SHADOW_GRANULARITY - end.value) <= end.offset) {
    *end.chunk = kAsanUserPoisonedMemoryMagic;
  }
...
void __asan_unpoison_memory_region(void const volatile *addr, uptr size) {
  ...
  Printf("[-] Unpoison memory region [%p, %p)\n    ", (void *)beg_addr, (void *)end_addr);
  Printf("%02x | ", *(end.chunk - 1));
  for (u8 *i = end.chunk; i <= end.chunk + 8; i++) Printf("%02x ", *i);
  ...
  if (beg.chunk == end.chunk) {
    ...
    // @acesrc: change unpoison
    // if (value != 0) {
    //   *beg.chunk = Max(value, end.offset);
    // }

    if (value != 0) {
      if (end.value >= 0 && end.value <= 8) *end.chunk = ASAN_SHADOW_GRANULARITY - Max((s8)(ASAN_SHADOW_GRANULARITY - end.value), end.offset);
      else *end.chunk = ASAN_SHADOW_GRANULARITY - end.offset;
    }
  ...
  // *end.chunk = Max(end.value, end.offset); // @acesrc: change unpoison
  // }
...
bool isContiguous(uptr shadow_beg, uptr size) {
  //@acesrc
  unsigned char shadowValue = *((unsigned char *)shadow_beg);
  if (shadowValue == 0) return __sanitizer::mem_is_zero((const char *)shadow_beg, size);

  if (shadowValue > 64) return false;
  uptr x = 1ULL << (64 - shadowValue);
  if (2 * x < size) return false;
  if (x >= size) return true;

  unsigned char newShadowValue = *((unsigned char *)(shadow_beg + size - x));
  if (shadowValue != newShadowValue) return false;
  return true;
}
...
uptr __asan_region_is_poisoned(uptr beg, uptr size) {
  ...
  volatile uptr __beg = beg;
  volatile uptr __end = end; 
  volatile uptr __shadow_beg = shadow_beg;
  volatile uptr __shadow_end = shadow_end;
  ...
  if (!__asan::AddressIsPoisoned(beg) && !__asan::AddressIsPoisoned(end - 1) &&
      ...
      isContiguous(shadow_beg, shadow_end - shadow_beg)))
  ...
  Printf("Region: %p %p", beg, end);
  Printf("Shadow: %p %p\n", shadow_beg, shadow_end);

  Printf("Failed Check: beg: %p, end: %p, size: %llu\n", beg, end, size);
  for (uptr i = MEM_TO_SHADOW(beg); i <= MEM_TO_SHADOW(end); i++) {
