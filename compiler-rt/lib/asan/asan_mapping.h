...
static inline bool AddressIsPoisoned(uptr a) {
  ...
  return shadow_value & 0x80;
  /*if (shadow_value) {
    u8 last_accessed_byte =
        (a & (ASAN_SHADOW_GRANULARITY - 1)) + kAccessSize - 1;
    // return (last_accessed_byte >= shadow_value); // @acesrc: change the poison check
    return shadow_value > ASAN_SHADOW_GRANULARITY - kAccessSize - (a & (ASAN_SHADOW_GRANULARITY - 1));
  }
  return false;*/
}
