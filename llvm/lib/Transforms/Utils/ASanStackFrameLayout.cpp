...
GetShadowBytes(const SmallVectorImpl<ASanStackVariableDescription> &Vars,
  ...
  //  SB.resize(SB.size() + Var.Size / Granularity, 0);
    // @acesrc: poisoning Stack
    size_t N = Var.Size / Granularity;
    size_t beg = SB.size();
    SB.resize(SB.size() + N, 0);
    size_t end = SB.size();
    int p = 0;
    ...
    //   SB.push_back(Var.Size % Granularity); // @acesrc: change the ASan Stack Poisoning
    if (Var.Size % Granularity)
      SB.push_back(Granularity - (Var.Size % Granularity));
  }
