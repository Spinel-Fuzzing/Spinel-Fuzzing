...
struct AddressSanitizer {
  AddressSanitizer(Module &M, const StackSafetyGlobalInfo *SSGI,
      ...
      : __spinel_M(M), CompileKernel(ClEnableKasan.getNumOccurrences() > 0 ? ClEnableKasan
                                                            : CompileKernel),
  ...
  void GiantSan_Eliminate(Function &F, FunctionAnalysisManager &FAM, SmallVector<InterestingMemoryOperand, 16> &interesting);
  void GiantSan_Merge(Function &F, FunctionAnalysisManager &FAM, SmallVector<InterestingMemoryOperand, 16> &interesting);
  void GiantSan_LoopSimplify(Function &F, FunctionAnalysisManager &FAM, SmallVector<InterestingMemoryOperand, 16> &interesting);
  ...
  bool instrumentFunction(Function &F, FunctionAnalysisManager &FAM, const TargetLibraryInfo *TLI);
...
PreservedAnalyses AddressSanitizerPass::run(Module &M,
  ...
  for (Function &F : M) {
    ...
    Modified |= FunctionSanitizer.instrumentFunction(F, FAM, &TLI);
  }
...
void AddressSanitizer::instrumentAddress(Instruction *OrigIns,
  ...
  Type *ShadowTy =
      IntegerType::get(*C, std::max(8U, TypeSize >> Mapping.Scale)); // TypeSize counts how many bits
  ...
  Type *MarkerType = ShadowTy;
  // GlobalVariable *__spinel_marker = __spinel_M.getGlobalVariable("__spinel_marker");
  // if (!__spinel_marker)
  // {
  //   __spinel_marker = new GlobalVariable(__spinel_M, MarkerType, false,
  //                                  GlobalValue::ExternalLinkage, 0, "__spinel_marker");
  // }
  //Value *__spinel_marker = this->__local_marker;

  size_t Granularity = 1ULL << Mapping.Scale;

  Value *marker = ShadowValue;
  if (ClAlwaysSlowPath || (TypeSize < 8 * Granularity)) {
    ...
    // RHS = Granularity - (Addr & (Granularity - 1)) - size
    Value *RHS = IRB.CreateSub(ConstantInt::get(IntptrTy, Granularity), LastAccessedByte);
    // (uint8_t) (RHS)
    RHS = IRB.CreateIntCast(RHS, ShadowValue->getType(), false);

    // ShadowValue > RHS -> Error!
    marker = IRB.CreateICmpUGT(ShadowValue, RHS);
*/


    /* Simplified */
    //Value *RHS = ConstantInt::get(MarkerType, 7);
    //marker = IRB.CreateICmpUGT(ShadowValue, RHS);
  }


  auto placeholder = __spinel_M.getOrInsertFunction("__spinel_placeholder__", IRB.getVoidTy(), MarkerType);
  IRB.CreateCall(placeholder, {IRB.CreateIntCast(marker, MarkerType, false, "")});



  if (getenv("INSERT_RUNTIME_CHECK")) {
     IRBuilder<> IRB(InsertBefore);
  size_t AccessSizeIndex = TypeSizeToSizeIndex(TypeSize);
  const ASanAccessInfo AccessInfo(IsWrite, CompileKernel, AccessSizeIndex);
  ...
  Value *Cmp = IRB.CreateICmpUGT(ShadowValue, ConstantInt::get(ShadowTy, 64));
  //IRB.CreateIsNotNull(ShadowValue);
  size_t Granularity = 1ULL << Mapping.Scale;
  ...
  if (ClAlwaysSlowPath || (TypeSize < 8 * Granularity)) {
    ...
    Value *Cmp2 = IRB.CreateICmpUGT(ShadowValue, ConstantInt::get(ShadowTy, 72));
  ...
  // Value *previous_value = IRB.CreateLoad(MarkerType, __spinel_marker);
  // Value *new_value = IRB.CreateOr(previous_value, IRB.CreateIntCast(marker, MarkerType, false, ""), "__spinel_new_marker");
  // IRB.CreateStore(new_value, __spinel_marker);
...
void AddressSanitizer::GiantSan_Eliminate(Function &F, FunctionAnalysisManager &FAM,SmallVector<InterestingMemoryOperand, 16> &OperandsToInstrument) { 
  if (getenv("SKIP_ELIMINATION")) return ;
...
void AddressSanitizer::GiantSan_Merge(Function &F, FunctionAnalysisManager &FAM,SmallVector<InterestingMemoryOperand, 16> &interesting) {

}

void AddressSanitizer::GiantSan_LoopSimplify(Function &F, FunctionAnalysisManager &FAM,SmallVector<InterestingMemoryOperand, 16> &interesting) {

}

bool AddressSanitizer::instrumentFunction(Function &F, FunctionAnalysisManager &FAM,
  ...
  /* Create Local Marker */ 
  {
    IRBuilder<> builder(F.getEntryBlock().getFirstNonPHIOrDbgOrLifetime(true));
    Type *ShadowTy = IntegerType::get(*C, 8);
    this->__local_marker = builder.CreateAlloca(ShadowTy);
    builder.CreateStore(ConstantInt::get(ShadowTy, 0), this->__local_marker);
  }
  ...
  // @acesrc: now we need to refine OperandsToInstrument by
  // - eliminate checks for safe mop
  // - merge the checks for aliased mop
  // - simplify checks inside loops
  
  GiantSan_Eliminate(F, FAM, OperandsToInstrument);
  GiantSan_Merge(F, FAM, OperandsToInstrument);
  GiantSan_LoopSimplify(F, FAM, OperandsToInstrument);
